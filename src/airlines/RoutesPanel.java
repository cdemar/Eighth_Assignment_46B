...
public class RoutesPanel extends JPanel {
	...
	RoutesPanel() {
		...
		setPreferredSize(new Dimension(usMapImage.getWidth() / 2,
				usMapImage.getHeight() / 2));
	...
	private class MLis extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			Airport clickedAirport = net.getAirportNearXY(e.getX(), e.getY(),
					CLICK_RADIUS);
	...
	public void paintComponent(Graphics g) {
		...
		// Existing routes in black. If an airport is armed, some will get
		// overdrawn.
		...
		if (armedAirport != null) {
			...
			for (Airport dest : armedAirport.getConnections())
				g.drawLine(armedAirport.getX(), armedAirport.getY(),
						dest.getX(), dest.getY());
		}
