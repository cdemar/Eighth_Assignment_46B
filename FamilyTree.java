...
public class FamilyTree {
	...
	public FamilyTree() throws IOException, TreeException {
		...
		// Parse the input file. Create a FileReader that reads treeFile. Create
		// a BufferedReader
	...
	private void addLine(String line) throws TreeException {
		...
		int colonIndex = line.indexOf(":"); // should be the index of the colon
											// in line.
		if (colonIndex < 0)

			// ?? throw a TreeException with a useful message
			throw new TreeException("Bad Hobbits!!!");

		// The substring of line that starts at char #0 and ends just before
		// colonIndex. Check the API for class java.util.String, method
		// substring(), if you need guidance.
		String parent = line.substring(0, colonIndex);

		// The substring of line that starts just after colonIndex and goes
		// through the end of the line. You'll use a different version of
		// substring().
		String childrenString = line.substring(colonIndex + 1);

		// Call childrenString.split(). Check the API for details. The result
		// will be an array of strings, with the separating commas thrown away.
	...
	// Returns the "deepest" node that is an ancestor of the node named name1,
	// and also is an ancestor of the node named name2.
	//
	// "Depth" of a node is the "distance" between that node and the root. The
	// depth of the root is 0. The depth of the root's immediate children is 1, and so on.
	//
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		ArrayList<TreeNode> ancestorsOf1 = node1.collectAncestorsToList(); //call the TreeNodes ancestor method on node1
	...
	public static void main(String[] args) {
		try {
			...
			TreeNode ancestor = tree.getMostRecentCommonAncestor("Bilbo",
					"Frodo");
			System.out
					.println("Most recent common ancestor of Bilbo and Frodo is "
							+ ancestor.getName());
		} catch (IOException x) {
